...
async function applyJob(req, res) {
    ...
    try{
        ...
        if(!profile.resume) {
            return res.status(400).json({success: false, message: "Upload resume in your profile"});
        }
...
async function getMyApplications(req, res) {
    ...
    try{    
        ...
        const applications = await applicationModel.find(filter).populate("job", "title location employmentType workMode minSalary maxSalary").populate("company", "name logo").sort(sortOption);
...
async function applicationStatus(req, res) {
    ...
    try{
        ...
        const application = await applicationModel.findById(applicationId).populate("job", "title minSalary maxSalary location jobType").populate("candidate", "name email avatar phone");
...
module.exports = { applyJob, getMyApplications, applicationStatus, getCompanyApplications, updateStatus, withdrawApplication };